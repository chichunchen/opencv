...
bool tracker_setup(CommandLineParser &, ifstream &, ifstream &, ifstream &, VideoCapture &, int &, int &, int &, int &,
                   unsigned int &, int &, int &, int &, bool &, double &);

bool isOOF(Point);

void process_mv_file(ifstream &, int, int, int, int, vector<vector<Point> > &, vector<vector<double> > &,
                     vector<vector<Point> > &, int *);
...
bool
process_det_bbox_vot2014(ifstream &, unordered_map<int, vector<Point2f> *> &, unordered_map<int, vector<Point2f> *> &);
...
double
interpolate_next_bbox(vector<Point2f> *&, vector<Point2f> *, Point2f *, vector<Point> &, vector<double> &, int, int,
                      int, int);
